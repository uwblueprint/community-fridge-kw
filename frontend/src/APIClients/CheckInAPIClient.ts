...
const getCheckInsByVolunteerId = async (volunteerId: string): Promise<CheckIn[]> => {
    try {
...
const deleteCheckInsByDateRange = async (startDate: string, endDate: string): Promise<boolean> => {
    try {
        await baseAPIClient.delete(`/checkin?startDate=${startDate}&endDate=${endDate}`, {
          headers: { Authorization: BEARER_TOKEN },
        });
        return true;
      } catch (error) {
